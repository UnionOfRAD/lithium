 ...
 */
class SharedMemory extends \lithium\core\Object {
    ...
    public function write($key, $data, $expiry = null) {
   		...
   		return function($self, $params) use ($expiry) {
   			...
   			if (is_array($key)) {
                $output = array();
                foreach($key as $k=>$v) {
                    $result = zend_shm_cache_store($k, $v, $cachetime);
                    if(!$result) {
                        $output[$k] = false;
                    }
                }
   			...
   			return zend_shm_cache_store($params['key'], $params['data'], $cachetime);
   		};
   	}
	...
	public function read($key) {
		return function($self, $params) {
            ...
            if(is_array($params['key'])) {
                ...
                return zend_shm_cache_fetch($params['key']);
            }
		};
	}
	...
	public function delete($key) {
		return function($self, $params) {
			...
			return zend_shm_cache_delete($params['key']);
		};
	}
