...
use lithium\storage\cache\adapter\zendserver\SharedMemory;

class SharedMemoryTest extends \lithium\test\Unit {
	...
	public function testSimpleWrite() {
		...
		$result = zend_shm_cache_delete($key);
		$this->assertTrue($result);
	...
	public function testWriteDefaultCacheTime() {
		$Cache = new SharedMemory(array('expiry' => '+5 seconds'));
		$key = 'key';
		$data = 'value';
	...
	public function testWriteMulti() {
		...
		$result = zend_shm_cache_fetch(array_keys($key));
		$this->assertEqual($key, $result);

        foreach($key as $k=>$v) {
            $result = zend_shm_cache_delete($k);
            $this->assertTrue($result);
        }
	}
	...
	public function testSimpleRead() {
		...
		$result = zend_shm_cache_store($key, $data, 60);
		$this->assertTrue($result);
	...
	public function testClear() {
		...
		$result = zend_shm_cache_store($key1, 'data that will no longer exist', 60);
		$this->assertTrue($result);

		$result = zend_shm_cache_store($key2, 'more dead data', 60);
		$this->assertTrue($result);

		$result = $this->Cache->clear();
		$this->assertTrue($result);

		$this->assertFalse(zend_shm_cache_fetch($key1));
		$this->assertFalse(zend_shm_cache_fetch($key2));
	}
}
