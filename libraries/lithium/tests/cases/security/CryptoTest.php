...
class CryptoTest extends \lithium\test\Unit {
	...
	public function testRandom64Generator() {
		...
		for ($i = 0; $i < $count; $i++) {
			$result = Crypto::random(8, array('encode' => Crypto::ENCODE_BASE_64));
	...
	/**
	 * Tests hash generation using `Crypto::hash()`.
	 *
	...
	public function testHash() {
		...
		$expected = sha1($value);
		$result = Crypto::hash($value, array('type' => 'sha1'));
		$this->assertEqual($expected, $result);

		$result = Crypto::hash($value, array('type' => 'sha1') + compact('salt'));
		$this->assertEqual(sha1($salt . $value), $result);
		$this->assertEqual(md5($value), Crypto::hash($value, array('type' => 'md5')));

		$result = Crypto::hash($value, array('type' => 'md5') + compact('salt'));
		...
		try {
			$result = Crypto::hash($value, array('type' => 'sha256'));
			$this->assertEqual($sha256($value), $result);

			$result = Crypto::hash($value, array('type' => 'sha256') + compact('salt'));
		...
		$result = Crypto::hash($string, compact('key'));
		$this->assertEqual($expected, $result);
